...
// Removed duplicate - using common.steps.ts version of "I click the {string} button"
...
When('I confirm the committee deletion', async function() {
  await this.page.locator('button:has-text("Confirm"), button:has-text("Yes")').click();
  await this.page.waitForTimeout(2000);
});

When('I click the {string} committee tab', async function(tabName: string) {
...
When('I click the {string} tab', async function(tabName: string) {
  // Generic tab click - same implementation as committee tab
  await this.page.locator(`mat-tab:has-text("${tabName}"), [role="tab"]:has-text("${tabName}")`).click();
  await this.page.waitForTimeout(1000);
});
