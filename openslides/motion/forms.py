...
class MotionCreateNewVersionMixin(forms.ModelForm):
    ...
    new_version = forms.BooleanField(
        required=False, label=_("Create new version (no trivial changes)"), initial=True,
...
class ConfigForm(CssClassMixin, forms.Form):
    ...
    motion_allow_trivial_changes = forms.BooleanField(
        label=_("Allow trivial changes without creating a new version"),
        required=False,
    )
