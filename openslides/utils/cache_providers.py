...
class BaseCacheProvider:
    ...
    async def add_changed_elements(self, args: Iterable[Union[int, str]]) -> None:
    ...
    async def get_lowest_change_id(self) -> List[Tuple[str, int]]:
...
class MemmoryCacheProvider(BaseCacheProvider):
    ...
    async def add_changed_elements(self, args: Iterable[Union[int, str]]) -> None:
        elements = list(args)
        if len(elements) % 2:
            raise ValueError("The argument of add_changed_elements has to be a list with an even number of elements.")

        for i in range(0, len(elements), 2):
            index = cast(int, elements[i])
            value = cast(str, elements[i+1])
            if elements[i] in self.change_id_data:
                self.change_id_data[index].add(value)
            else:
                self.change_id_data[index] = {value}
    ...
    async def get_lowest_change_id(self) -> List[Tuple[str, int]]:
        change_data = self.change_id_data
        if change_data:
            return [('no_usefull_value', min(change_data.keys()))]
        return []
